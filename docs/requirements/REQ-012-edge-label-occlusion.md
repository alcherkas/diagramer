...
- [ ] An edge label whose position falls inside a node's bounds still receives
      clicks and double-clicks (label layer stacks above unselected nodes).
- [ ] A selected node still raises above edge labels (React Flow's
      elevate-on-select keeps working) so dragging isn't obstructed.
- [ ] e2e: seed a diagram where the label midpoint is under a node; double-
